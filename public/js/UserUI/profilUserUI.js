...
function EditCard(editButton) {
   ...
   let user_heslo = content_editable_5.value;
   ...
   if (button_text === "Upraviť Profil") {
      ...
      if (overCiSuZadaneUdaje(meno, email, user_heslo, tel_cislo)) {
         zapniNormalnyMod();
         editUserData(
   ...
   function zapniEditacnyMod() {
      ...
      content_editable_3.setAttribute("contenteditable", "false"); //email by nemal môcť editovať
...
/*  backend funkcie */
function editUserData(id, meno, email, user_heslo, tel_cislo) {
   alert(user_heslo);
}
